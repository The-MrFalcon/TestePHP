...
class DataRequest {
    ...
    /**
     * Função que retorna os dados dos Menu ou a quantidade de registros
     * @param string recebe 'c' ou vazio
     * @return array||String
     */
    public function dadosMenu($param = '') {
        if($param === 'c') {
            return count($this->DadosMenu);
        } else {
            return $this->DadosMenu;
        }
    }
    public function dadosRelatorio($param = '') {
        if($param === 'c') {
            return count($this->DadosRelatorio);
        } else {
